...
/**
 * Legacy transition state interface for backward compatibility.
 *
 * This file contains the old TransitionState implementation that was used
 * before the tree-based navigation refactoring. It will be removed once
 * all navigators are migrated to use the new NavNode-based [TransitionState].
 *
 * @see TransitionState for the new tree-aware implementation
 */
sealed interface LegacyTransitionState {

    /**
     * No transition is occurring.
     */
    data object Idle : LegacyTransitionState
 ...
 */
val LegacyTransitionState.isAnimating: Boolean
    get() = this !is LegacyTransitionState.Idle

/**
 * Extension to get the current progress, regardless of transition type.
 * Returns 0 for [LegacyTransitionState.Idle].
 */
val LegacyTransitionState.progress: Float
    get() = when (this) {
        is LegacyTransitionState.Idle -> 0f
        is LegacyTransitionState.InProgress -> progress
        is LegacyTransitionState.PredictiveBack -> progress
        is LegacyTransitionState.Seeking -> progress
    }
