...
internal fun NavNodeRenderer(
    ...
    when (node) {
        is ScreenNode -> {
            // ScreenRenderer should be called from within an AnimatedVisibilityScope context
            // The AnimatedVisibilityScope is provided by the parent renderer (StackRenderer, etc.)
            // via AnimatedNavContent or similar animation components
            ScreenRenderer(
                node = node,
                scope = scope,
            )
