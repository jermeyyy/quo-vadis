 ...
 * @property modulePrefix Optional prefix for generated class names (e.g., "ComposeApp" -> "ComposeAppDeepLinkHandlerImpl")
 */
class DeepLinkHandlerGenerator(
     ...
     *        pointing users to use NavigationConfig instead
     */
    ...
    private fun buildHandlerObject(destinations: List<DestinationInfo>, addDeprecations: Boolean): TypeSpec {
        return TypeSpec.objectBuilder(handlerName)
            ...
            // Legacy DeepLinkHandler interface methods (deprecated)
            .addFunction(buildLegacyRegisterFunction())
            .addFunction(buildLegacyHandleFunction())
            .build()
    }
    ...
    private fun buildDeprecationAnnotation(): AnnotationSpec {
        return AnnotationSpec.builder(Deprecated::class)
            ...
            .addMember(
                "replaceWith = %T(%S, %S)",
                ReplaceWith::class,
                "NavigationConfig.deepLinkHandler",
                "$GENERATED_PACKAGE.NavigationConfig"
            )
     ...
     * 
     * Uses resolve(uri) to find a matching destination.
     */
    ...
    /**
     * Build the legacy register function from DeepLinkHandler interface.
     * 
     * This is a no-op for generated handlers since routes are determined at compile time.
     * Signature: register(pattern: String, action: (Navigator, Map<String, String>) -> Unit)
     */
    private fun buildLegacyRegisterFunction(): FunSpec {
        val actionType = LambdaTypeName.get(
            parameters = listOf(
                ParameterSpec.builder("navigator", NAVIGATOR).build(),
    ...
    /**
     * Build the legacy handle function from DeepLinkHandler interface.
     * 
     * Signature: handle(deepLink: DeepLink, navigator: Navigator)
     */
    private fun buildLegacyHandleFunction(): FunSpec {
        return FunSpec.builder("handle")
            .addModifiers(KModifier.OVERRIDE)
            .addParameter("deepLink", DEEP_LINK)
            .addParameter("navigator", NAVIGATOR)
