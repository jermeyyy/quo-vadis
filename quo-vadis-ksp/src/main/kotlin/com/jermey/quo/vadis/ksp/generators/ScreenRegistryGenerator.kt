...
class ScreenRegistryGenerator(
    ...
    companion object {
        ...
        private val SHARED_TRANSITION_SCOPE =
            ClassName("androidx.compose.animation", "SharedTransitionScope")
        private val ANIMATED_VISIBILITY_SCOPE =
     ...
     */
    fun generate(screens: List<ScreenInfo>, addDeprecations: Boolean = false) {
        if (screens.isEmpty()) {
            logger.warn("No @Screen annotations found, skipping registry generation")
            return
        }

        val fileSpec = buildFileSpec(screens, addDeprecations)
    ...
    private fun buildFileSpec(screens: List<ScreenInfo>, addDeprecations: Boolean): FileSpec {
        return FileSpec.builder(GENERATED_PACKAGE, REGISTRY_NAME)
            ...
            .apply {
                screens.forEach { screenInfo ->
                    // Find the top-level class (first class in the hierarchy without a class parent)
                    val topLevelClass = findTopLevelClass(screenInfo.destinationClass)
                    println("screenInfo: $screenInfo")
                    println("screenInfo.destinationClass.parentDeclaration: ${screenInfo.destinationClass.parentDeclaration}")
                    println("screenInfo.destinationClass: ${screenInfo.destinationClass}")
                    println("DECLARATION: ${topLevelClass.packageName.asString()}.${topLevelClass.simpleName.asString()}")
                    addImport(
                        packageName = topLevelClass.packageName.asString(),
                        names = listOf(topLevelClass.simpleName.asString())
                    )
                    addImport(
                        packageName = screenInfo.packageName,
                        names = listOf(screenInfo.functionName)
                    )
                }
            }
            .addType(buildRegistryObject(screens, addDeprecations))
            .build()
    }
    ...
    private fun buildRegistryObject(screens: List<ScreenInfo>, addDeprecations: Boolean): TypeSpec {
    ...
    private fun buildDestinationClassName(screen: ScreenInfo): String {
        // Build full nested class path from top-level class
        // e.g., MainTabs.SettingsTab.SettingsMain
        val names = mutableListOf<String>()
        var current: KSDeclaration? = screen.destinationClass
