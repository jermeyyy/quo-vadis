...
object GraphGenerator {
    ...
    private fun buildFileSpec(
        ...
        // 1. Use explicitly specified startDestination from @Graph annotation
        // 2. Fall back to first object destination
        // 3. Fall back to first destination
        val startDestination = when {
            !graphInfo.startDestinationName.isNullOrEmpty() -> {
                // Validate that the specified start destination exists
                val destExists = graphInfo.destinations.any { it.name == graphInfo.startDestinationName }
                if (!destExists) {
                    error("Start destination '${graphInfo.startDestinationName}' not found in ${graphInfo.className}")
                }
                graphInfo.startDestinationName
            }
            else -> {
                graphInfo.destinations
